...
def parseModel(path, ns):
    ...
    for planItem in planItems:
        ...
        reqRules = planItem.findall(
            ".//cmmn:itemControl/cmmn:requiredRule", namespaces=ns)
        repRules = planItem.findall(
            ".//cmmn:itemControl/cmmn:repetitionRule", namespaces=ns)
        mandatory = len(reqRules) > 0
        repetition = len(repRules) > 0
        ...
        if defRef.startswith("Task"):
            typ = 'Task'
            labelNode = root.findall(
                f".//cmmn:task[@id='{defRef}']", namespaces=ns)[0]
            label = labelNode.attrib['name']
        elif defRef.startswith("Stage"):
            typ = 'Stage'
            stage = root.findall(
                f".//cmmn:stage[@id='{defRef}']", namespaces=ns)[0]
            label = stage.attrib['name']
            if label == "":
                stageCnt += 1
                label = f'Stage{stageCnt}'
            stages.append((stage, itemObj))
        elif defRef.startswith("Milestone"):
            typ = 'Milestone'
            milestone = root.findall(
                f".//cmmn:milestone[@id='{defRef}']", namespaces=ns)[0]
        ...
        for entryCrit in planItem.findall("cmmn:entryCriterion", namespaces=ns):
            # print("- sentry")
            sentry = root.findall(
                f".//cmmn:sentry[@id='{entryCrit.attrib['sentryRef']}']", namespaces=ns)[0]
            ...
            for itemPart in itemParts:
                ...
                sourcePlanItem = root.findall(
                    f".//cmmn:planItem[@id='{itemPart.attrib['sourceRef']}']", namespaces=ns)[0]
                sourceDefRef = sourcePlanItem.attrib['definitionRef']
                sentryItemObj['source'] = sourceDefRef
                # print("source:", sourceDefRef)

                events = itemPart.findall(
                    ".//cmmn:standardEvent", namespaces=ns)
                if len(events) > 0:
            ...
            associations = root.findall(
                f".//cmmn:association[@sourceRef='{entryCrit.attrib['id']}']", namespaces=ns)
            for association in associations:
                condItemObj = {'id': association.attrib['targetRef']}
                textAnnotations = root.findall(
                    f".//cmmn:textAnnotation[@id='{association.attrib['targetRef']}']", namespaces=ns)
                if len(textAnnotations) > 0:
                    textAnnotation = textAnnotations[0].findall(
                        ".//cmmn:text", namespaces=ns)[0].text
